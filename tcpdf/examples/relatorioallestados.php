...
function listacidade()
{
  ...
  $link = mysqli_connect($servername, $username, $password, $dbname);
	...
	$sql = "SELECT A.ID, A.IDESTADO, B.NOME as NOMEESTADO, A.NOME as NOMECIDADE FROM A003_CIDADES as A
inner join A002_ESTADOS as B on A.IDESTADO = B.ID ";  
  ...
  while ($Dados = $res->fetch_assoc()) {
    ...
    $id[$x] = $Dados["ID"];
    $idestado[$x] = $Dados["IDESTADO"]; 
    $nomeestado[$x] = $Dados["NOMEESTADO"];
    $nomecidade[$x] = $Dados["NOMECIDADE"]; 
		$x++;
	}   
  ...
  $celula_Lvalor = ' style="font-weight: normal;
    ...
    $fhtml .= '<tr>
					<td' .$celula_Litem .'>'.$id[$x].'</td>
					<td' .$celula_Lcodigo .'>' .$idestado[$x].'</td>
					<td' .$celula_Lnome .'>' .utf8_encode($nomeestado[$x]) .'</td>
					<td' .$celula_Lvalor . '> '.utf8_encode($nomecidade[$x]).'</td>
				</tr>';       
    }	
...
function listaestado()
{
  ...
  $fhtml .= '<div style="padding-left:200px;">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td' .$celula_Titem .'>ID</td>
					<td' .$celula_Tcodigo .'>IDESTADO</td>
					<td' .$celula_Tnome .'>NOMEESTADO</td>
					<td' .$celula_Tvalor .'>NOMECIDADE </td>
				  </tr>';          
