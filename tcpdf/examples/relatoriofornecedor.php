...
function listaprodutos($idfornecedor, $item)
{
  ...
  while ($Dados = $res->fetch_assoc()) {
    ...
    $id[$x] = $Dados["ID"];
		...
		$nomeproduto[$x] = $Dados["NOMEDOPRODUTO"];
		$imagem[$x] = base64_encode($Dados["IMAGEM"]);
		$comissao[$x] = $Dados["COMISSAO"];
		$frete[$x] = $Dados["FRETE"];
		$und[$x] = $Dados["UND"];
		$marca[$x] = $Dados["MARCA"];
		$cor[$x] = $Dados["COR"];
		$categoria[$x] = $Dados["CATEGORIA"];
		$subcategoria[$x] = $Dados["SUBCATEGORIA"];
		$fornecedor[$x] = $Dados["FORNECEDOR"];
		$precovenda[$x] = $Dados["PRECOVENDA"];  
  ...
  $celula_Lvalor = ' style="font-weight: normal;
    ...
    $fhtml .= '<tr>
					<td' .$celula_Litem .'>'.utf8_encode(str_pad($item, 5, "0", STR_PAD_LEFT)).'</td>
					<td' .$celula_Lcodigo .'>' .utf8_encode($codigo[$x]) .'</td>
					<td' .$celula_Lnome .'>' .utf8_encode($nomeproduto[$x]) .'</td>
					<td' .$celula_Lvalor . '>R$ '.number_format($precovenda[$x],2,",",".").'</td>
				</tr>';
       $item++;
    }	
...
function listafornecedor($id)
{
 ...
 // }
  ...
  while ($Dados = $res->fetch_assoc()) {   		
    ...
    $id[$x] = $Dados["ID"];
    $md5_id[$x] = md5($Dados["ID"]);
    $nome[$x] = $Dados["FORNECEDOR"];       
		$x++;
	} 
  ...
  $sql = "SELECT ID, UPPER(NOME) AS FORNECEDOR
  	FROM C001_PESSOAS
	  WHERE TIPO = 2 AND ID IN (SELECT DISTINCT IDFORNECEDOR FROM E005_PRODUTOS WHERE ATIVO = 1 AND IDFORNECEDOR = " .$foundpID .")
	  ORDER BY NOME";  
  ...
  $fhtml .= '<div class="grupo">' .utf8_encode($nome[$x]) .'</div>
				<div style="padding-left:200px;">
        
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td' .$celula_Titem .'>ÍTEM</td>
					<td' .$celula_Tcodigo .'>CÓDIGO</td>
					<td' .$celula_Tnome .'>NOME</td>
					<td' .$celula_Tvalor .'>VALOR R$</td>
				  </tr>';
          
          $item = 1;
          
         $fhtml .= listaprodutos($id[$x], $item++);
